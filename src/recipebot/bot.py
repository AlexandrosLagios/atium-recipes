...
def preview_markup(token: str, plan: IngredientPlan, language: str) -> InlineKeyboardMarkup:
    row = [InlineKeyboardButton(t(language, "save_button"), callback_data=f"save:{token}")]
    if plan.near:
        row.append(
            InlineKeyboardButton(t(language, "merge_button"), callback_data=f"merge:{token}")
        )
    row.append(InlineKeyboardButton(t(language, "drop_button"), callback_data=f"drop:{token}"))
...
async def send_preview(recipe, plan, vocab, update, language: str, page_id: str = "") -> None:
    ...
    sent = await update.effective_message.reply_text(
        preview_text(recipe, plan, language),
        reply_markup=preview_markup(token, plan, language),
    )
...
async def apply_correction(token: str, instruction: str, update, context) -> None:
    ...
    await context.bot.edit_message_text(
        ...
        reply_markup=preview_markup(token, plan, preview.language),
    )
        ...
        with contextlib.suppress(BadRequest):
            await query.edit_message_text(
                ...
                reply_markup=preview_markup(
                    token, IngredientPlan(near=preview.merges), preview.language
                ),
            )
        return
