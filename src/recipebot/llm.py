...
- Method steps are whole sentences in order. Name an ingredient in a step exactly as the ingredient list names it, so the step can link to it: "the chicken breast", never "the chicken". A method step obeys every quantity rule above, so never copy a measure out of the source unconverted: "Heat oven to 425°F" becomes "Heat the oven to 220°C". A step is an instruction, never a heading the source uses to group its steps: drop "How To Make The Stir-Fry Sauce:" and keep the instructions under it.
